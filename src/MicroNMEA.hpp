 ...
 * @file MicroNMEA.h
 * @author Steve Marple
 *  @author changes by Vinnie Moscaritolo
 */
...
public:
	...
	MicroNMEA(void* buffer, uint8_t len, void* ctx);
	 ...
	 */
	void setBadChecksumHandler(void (*handler)(MicroNMEA& nmea, void* ctx)) {
	 ...
	 * @param handler pointer to handler function
	 */
	void setUnknownSentenceHandler(void (*handler)(MicroNMEA& nmea, void* ctx)) {
...
protected:
	...
	bool processGSV(const char* s);
	bool processVTG(const char* s);
...
private:
	...
	void (*_badChecksumHandler)(MicroNMEA &nmea, void* ctx);
	void (*_unknownSentenceHandler)(MicroNMEA &nmea, void* ctx);
	void* _context;
